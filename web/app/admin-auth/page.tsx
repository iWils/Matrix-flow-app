...
export default function AuthConfigPage() {
  ...
  async function testConnection(type: 'ldap' | 'oidc') {
    try {
      ...
      alert(t('admin:connectionTestError'))
    }
  }
  ...
  return (
    <div className="animate-fade-in">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gradient mb-2">{t('common:authConfiguration')}</h1>
      ...
      <Card className="mb-6">
        <div className="p-6">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Fournisseurs d&apos;authentification</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {providers.map(provider => (
              <div key={provider.id} className="flex items-center justify-between p-3 bg-slate-700 rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-400 uppercase">{provider.type}</p>
                </div>
            ...
            {providers.length === 0 && (
              <div className="col-span-3 text-center py-4">
                <p className="text-sm text-slate-400 dark:text-slate-400">Aucun fournisseur configuré</p>
              </div>
            )}
          </div>
        </div>
